...
import './ERC20Dynamic.sol';
import '../interfaces/IERC20.sol';
import '../interfaces/IFactoryDynamic.sol';
import '../interfaces/ICallee.sol';
import './FeeVault.sol';

contract PairDynamic is ERC20Dynamic {
    ...
    function initialize(address _token0, address _token1, uint _fee, bool _stable) external {
        ...
        fees = address(new FeeVault(_token0, _token1));
        ...
        if (_stable) {
            symbol = string(abi.encodePacked("sLP (", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol(), ')'));
            name = string(abi.encodePacked("Stable LP (", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol(), ')'));
        } else {
            symbol = string(abi.encodePacked("vLP (", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol(), ')'));
            name = string(abi.encodePacked("Volatile LP (", IERC20(_token0).symbol(), "/", IERC20(_token1).symbol(), ')'));
        }
    ...
    function changeFeeType(uint _fee) external {
        ...
        if(voteWeight >= totalSupply.div(2)){
          ...
          _transfer(msg.sender, IFactoryDynamic(factory).feeTo(), liqFee);
    ...
    function getProtocolFee(uint amount, address to) internal view returns (uint feeAmount){
        if(IFactoryDynamic(factory).feeTo() == to){
          feeAmount = 0;
          return feeAmount;
        }
        // if dynamic adjusted fees are on, use that instead of fixed fee
        if(IFactoryDynamic(factory).protocolFeeDynamic() != 0) {
          feeAmount = amount.mul(pairFee.mul(IFactoryDynamic(factory).protocolFeeDynamic())).div(1000 * 10000);
        } else {
          feeAmount = amount.mul(IFactoryDynamic(factory).protocolFeeFixed()).div(10000);
        }
    }

    function chargeProtocolFees() internal view returns (bool) {
        return IFactoryDynamic(factory).protocolFeeFixed() != 0 || IFactoryDynamic(factory).protocolFeeDynamic() != 0;
    }
    ...
    function claimFees() external returns (uint claimed0, uint claimed1) {
        ...
        if (claimed0 > 0 || claimed1 > 0) {
            ...
            FeeVault(fees).claimFeesFor(msg.sender, claimed0, claimed1);
    ...
    function _update0(uint amount, uint pAmount) internal {
        ...
        if(pAmount >= amount){
          _safeTransfer(token0, IFactoryDynamic(factory).feeTo(), amount);
          emit Fees(msg.sender, 0, 0, amount, 0);
        } else {
          _safeTransfer(token0, IFactoryDynamic(factory).feeTo(), pAmount);
    ...
    function _update1(uint amount, uint pAmount) internal {
        // all fees must be paid to protocol if charging less than standard
        if(pAmount >= amount){
          _safeTransfer(token1, IFactoryDynamic(factory).feeTo(), amount);
          emit Fees(msg.sender, 0, 0, 0, amount);
        } else {
          _safeTransfer(token1, IFactoryDynamic(factory).feeTo(), pAmount);
